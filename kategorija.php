...
<html>
  <head>
      <title>
      <?php
      $category_q = mysqli_query($connection, "SELECT ime_kategorije,opis_kategorije FROM kategorija WHERE id_kategorije = '$cid'");
while($c_row = mysqli_fetch_array($category_q))
{
    $c_desc = $c_row['opis_kategorije'];
    $c_name = $c_row['ime_kategorije'];
    echo $c_name . "-Trgovina građevinskog materijala";
}
    ?>
    </title>
    ...
    <div id="page-wrapper">
        ...
        <nav>
          ...
          <ul>
           ...
           <div class="drop-down">
            <li class="active"><a href="ponuda.php" class="drop-down-link">PONUDA</a></li>
      ...
      <div class="space">&nbsp;</div>
      <?php
      $category_description=mysqli_query($connection,"SELECT ime_kategorije,opis_kategorije FROM kategorija WHERE id_kategorije='$cid'");
      while($description_row = mysqli_fetch_array($category_description))
      {
        $description = $description_row['opis_kategorije'];
        $category_name = $description_row['ime_kategorije'];
      ?>
      <div class="category-container">
       <div class="category-name"><h2><?php echo $category_name; ?></h2></div>
       <br>
       <div class="category-description"><p><?php echo $description; ?></p></div>
      </div>
      <?php
      }
      ?>
      ...
      <?php
      $items_query=mysqli_query($connection,"SELECT * FROM proizvod WHERE id_kategorije='$cid'");
      while($item_row = mysqli_fetch_array($items_query))
      {
        $opis = $item_row['opis_proizvod'];
        $slika = $item_row['slika_proizvod'];
        $image_id = $item_row['proizvod_id'];
        ?>
      <div class="item">
        <div class="item-image"><img src="proizvodi/<?php echo $slika;?>" width="300" height="200" alt="<?php echo $opis; ?>-Domgrad" /></div>
       ...
       <div class="image-popup">
        <img src="proizvodi/<?php echo $slika; ?>" width="400" height="300"/>
      </div>
     <?php
      }
      ?>
